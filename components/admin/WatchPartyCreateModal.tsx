...
import { Activity, Check, Loader2, Search, Tv, X } from 'lucide-react';
...
export function WatchPartyCreateModal({onClose,onCreated,onImported}:{onClose:()=>void;onCreated:(party:AdminParty)=>void;onImported:(result:{createdCount:number;skippedCount:number})=>void}) {
  ...
  const importF1=async()=>{
    ...
    try{
      ...
      const response=await fetch('/api/admin/watch-parties/f1-calendar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({raceIds:selectedRaces,entryFeeRupees:fee,entryCoins:reward,driverMultipliers:driverMultiplierBasisPoints})});
  ...
  return <AdminBookingModalShell onClose={()=>!busy&&onClose()} labelledBy="create-watch-party-title">
    ...
    {reviewing&&catalog?<div className="wp-review">
      ...
      {format==='TEAM'?<>
        ...
        <fieldset><legend>Driver Reward Multipliers</legend><div className="wp-driver-search"><Search size={15}/><input className="form-input" type="search" placeholder="Search driver or team" value={driverSearch} onChange={event=>setDriverSearch(event.target.value)}/></div><div className="wp-drivers">{visibleDrivers.map(driver=><label key={driver.key}><span><strong>{driver.name}</strong><small>{driver.team}</small></span><span className="wp-multiplier"><input className="form-input" inputMode="decimal" value={driverMultipliers[driver.key]??''} onChange={event=>setDriverMultipliers(current=>({...current,[driver.key]:event.target.value}))} aria-label={`${driver.name} reward multiplier`} aria-invalid={predictionOddsBasisPoints(driverMultipliers[driver.key]??'')==null}/><b>x</b></span></label>)}</div></fieldset>
      </>}
