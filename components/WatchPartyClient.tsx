...
function partyStatus(party: WatchPartySummary, signedIn: boolean) {
  ...
  return { label: party.prediction ? 'Predicted' : 'Entered', tone: 'success' };
}
...
export function WatchPartyClient({
  ...
  const buyShopOrder = async (itemKey: string) => {
    ...
    setShopOrderOverlay({
      status: 'loading',
      label: selectedItem?.label ?? 'Ticket',
    ...
    try {
      ...
      const data = await readApiResponse<WatchPartyShop & { error?: string }>(res, 'Order failed.');
      ...
      setShopOrderOverlay({
        status: 'success',
        label: latestOrder?.label ?? selectedItem?.label ?? 'Ticket',
    ...
    <>
      <section className="watch-party-shell">
        <div className="watch-party-top">
          <div>
            <div className="watch-kicker">Premier League 2026-27</div>
            <h1 className="font-orbitron">Watch Party</h1>
          </div>
        ...
        <div className="watch-list">
          {state.parties.length === 0 ? (
            <div className="watch-empty">
              ...
              <span>Check back when the next fixture opens.</span>
            </div>
      ...
      <section className="watch-shop">
        <div className="watch-rewards-strip">
          <div>
            <div className="watch-kicker">Rewards</div>
            <h2 className="font-orbitron">Shop</h2>
            <p>{pendingOrderCount} counter ticket{pendingOrderCount === 1 ? '' : 's'}</p>
          </div>
          <div className="watch-rewards-actions">
            <div className="watch-shop-wallet">◈ {walletCoins ?? '--'}</div>
        ...
        <div className="watch-shop-backdrop" role="dialog" aria-modal="true" onClick={() => setShopOpen(false)}>
          <div className="watch-shop-sheet" onClick={(event) => event.stopPropagation()}>
            <div className="watch-shop-sheet-head">
              ...
              <button className="watch-modal-close" type="button" onClick={() => setShopOpen(false)} aria-label="Close shop">
                <X size={18} />
              </button>
            </div>

            <div className="watch-shop-tabs" role="tablist" aria-label="Shop sections">
              <button type="button" className={shopTab === 'passes' ? 'active' : ''} onClick={() => setShopTab('passes')}>Passes</button>
              <button type="button" className={shopTab === 'drinks' ? 'active' : ''} onClick={() => setShopTab('drinks')}>Drinks</button>
            ...
            <div className="watch-shop-body">
              {shopTab === 'tickets' ? (
                !signedIn ? (
                  <p className="watch-shop-empty">Login to see your counter tickets.</p>
                ) : state.shop.orders.length === 0 ? (
                  <p className="watch-shop-empty">No counter tickets yet.</p>
                ) : (
                      ...
                      <article className="watch-pass-ticket" key={order.id}>
                        <div>
                          <div className="watch-pass-ticket-top">
                            <span><TicketCheck size={14} /> {order.status === 'PENDING' ? 'Counter Ticket' : 'Given'}</span>
                            {order.status === 'GIVEN' && <CheckCircle2 size={15} />}
                          </div>
                          <h3>{order.label}</h3>
                          <p>{order.category} · ◈ {order.tokenCost}</p>
                        </div>
                        <div className="watch-pass-ticket-foot">
                          <span>{order.status === 'PENDING' ? 'Show counter' : 'Given'}</span>
                        </div>
                      </article>
                    ))}
                  </div>
                    ...
                    return (
                      <article className={`watch-shop-card ${item.accent}`} key={item.itemKey}>
                        <div className="watch-shop-main">
                          ...
                          <div>
                            <h3>{item.label}</h3>
                            <p>{item.detail} · {item.category}</p>
                          </div>
                        </div>
                        <div className="watch-shop-action">
                          <strong>◈ {item.tokenCost}</strong>
                          {!signedIn ? (
                            <Link className="btn btn-primary btn-sm" href="/login">
                              <LogIn size={14} />
                              Login
                            ...
                            >
                              <ShoppingBag size={14} />
                              {shopBusy === item.itemKey ? 'Getting' : canBuy ? 'Get' : 'Need'}
                            </button>
                          )}
                        </div>
                      </article>
      ...
      {shopOrderOverlay && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={shopOrderOverlay.status === 'loading' ? 'Acquiring ticket' : 'Ticket acquired'}
          onClick={() => {
          ...
          <section className="watch-order-card" onClick={(event) => event.stopPropagation()}>
            {shopOrderOverlay.status === 'loading' ? (
              <>
                ...
                <div>
                  <h2 className="font-orbitron">Acquiring Ticket</h2>
                  <p>{shopOrderOverlay.label}</p>
                </div>
                <strong>{shopOrderOverlay.tokenCost ? `◈ ${shopOrderOverlay.tokenCost}` : 'Arena Tokens'}</strong>
              </>
            ) : (
              <>
                <span className="watch-order-check"><TicketCheck size={24} /></span>
                <div>
                  <h2 className="font-orbitron">Ticket Acquired</h2>
                  <p>{shopOrderOverlay.label}</p>
                </div>
      ...
      {enteringParty && (
        <div className="watch-modal-backdrop" role="dialog" aria-modal="true" onClick={() => setEnteringParty(null)}>
          <div className="watch-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <h2>Check-in credited</h2>
            <p>{enteringParty.homeTeam} vs {enteringParty.awayTeam}</p>
            <strong className="watch-token-big">◈ {enteringParty.entryCoins}</strong>
            <p>Entering unlocks this watch party and its predictions.</p>
            {error && <div className="watch-alert">{error}</div>}
      ...
      <style jsx>{`
        ...
        .watch-rewards-actions { flex: 0 0 auto; justify-content: flex-end; }
        .watch-shop-wallet { min-height: 38px; display: grid; place-items: center; padding: 0 12px; border: 1px solid rgba(34,211,238,0.3); border-radius: 8px; color: #22d3ee; background: rgba(34,211,238,0.09); font-family: Orbitron, sans-serif; font-weight: 800; }
        .watch-shop-backdrop { position: fixed; inset: 0; z-index: 1100; display: flex; align-items: flex-end; justify-content: center; padding: 12px; background: rgba(0,0,0,0.72); }
        .watch-shop-sheet { position: relative; width: min(100%, 540px); height: min(82vh, 720px); height: min(82dvh, 720px); max-height: calc(100vh - 24px); display: grid; grid-template-rows: auto auto minmax(0, 1fr); gap: 10px; padding: 14px; border: 1px solid rgba(148,163,184,0.22); border-radius: 12px 12px 8px 8px; background: #070a12; box-shadow: 0 -18px 40px rgba(0,0,0,0.42); overflow: hidden; }
        ...
        .watch-shop-tabs button { min-height: 34px; border: 0; border-radius: 6px; color: var(--color-text-muted); background: transparent; font-size: 0.74rem; font-weight: 900; }
        ...
        .watch-order-overlay { position: fixed; inset: 0; z-index: 1250; display: grid; place-items: center; padding: 18px; background: rgba(0,0,0,0.62); }
        .watch-order-card { width: min(100%, 330px); min-height: 230px; display: grid; justify-items: center; align-content: center; gap: 14px; padding: 18px; border: 1px solid rgba(34,211,238,0.34); border-radius: 8px; background: #050914; box-shadow: 0 18px 46px rgba(0,0,0,0.46); text-align: center; }
        ...
        .watch-shop-action { min-width: 140px; flex: 0 0 auto; justify-content: flex-end; }
        .watch-shop-action strong { min-width: 62px; color: #d8fbff; font-family: Orbitron, sans-serif; font-size: 0.82rem; text-align: right; }
        .watch-shop-action .btn { min-width: 70px; justify-content: center; padding-inline: 9px; }
        ...
        .watch-pass-ticket { min-height: 68px; justify-content: space-between; padding: 10px 11px; border: 1px solid rgba(34,211,238,0.18); border-left: 3px solid #22d3ee; border-radius: 8px; background: rgba(5,12,24,0.82); }
        ...
        .watch-icon-btn { width: 42px; height: 42px; display: grid; place-items: center; border: 1px solid rgba(34,211,238,0.35); border-radius: 8px; background: rgba(34,211,238,0.09); color: #d8fbff; }
        .watch-spin { animation: watchSpin 0.8s linear infinite; }
        .watch-wallet { min-height: 58px; display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 12px 14px; margin-bottom: 14px; border: 1px solid rgba(34,211,238,0.3); border-radius: 8px; background: rgba(2,8,16,0.72); }
        .watch-wallet span { color: var(--color-text-muted); font-size: 0.82rem; }
        .watch-wallet strong, .watch-token-big { color: #22d3ee; font-family: Orbitron, sans-serif; }
        ...
        .watch-modal-backdrop { position: fixed; inset: 0; z-index: 1100; display: grid; place-items: center; padding: 18px; background: rgba(0,0,0,0.72); }
        .watch-modal { width: min(100%, 360px); position: relative; display: grid; gap: 12px; padding: 20px; border: 1px solid rgba(34,211,238,0.34); border-radius: 8px; background: #050914; text-align: center; }
        .watch-modal h2 { margin: 0; font-size: 1.2rem; }
        .watch-modal p { margin: 0; color: var(--color-text-secondary); }
        .watch-token-big { font-size: 1.8rem; }
        .watch-modal-close { position: absolute; top: 10px; right: 10px; width: 34px; height: 34px; display: grid; place-items: center; color: var(--color-text-muted); background: transparent; border: 0; }
        ...
        @media (max-width: 560px) {
          ...
          .watch-shop-sheet { height: min(84vh, 640px); height: min(84dvh, 640px); max-height: calc(100vh - 16px); padding: 12px; }
          .watch-rewards-strip { align-items: stretch; flex-direction: column; }
          .watch-rewards-actions { justify-content: space-between; }
