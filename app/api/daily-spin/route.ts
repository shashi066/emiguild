...
export async function GET() {
  ...
  try {
    ...
    const encryptedSpin = spin ? {
      ...spin,
      lootItem: spin.lootItem ? { ...spin.lootItem, weight: encryptNumber(spin.lootItem.weight) } : null
    } : null;
...
export async function POST() {
  ...
  try {
    ...
    const encryptedReward = { ...selectedItem, weight: encryptNumber(selectedItem.weight) };
    const encryptedSpinRecord = {
      ...spinRecord,
      lootItem: spinRecord.lootItem ? { ...spinRecord.lootItem, weight: encryptNumber(spinRecord.lootItem.weight) } : null
    };
